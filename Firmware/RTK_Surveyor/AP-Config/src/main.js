...
function parseIncoming(msg) {
  ...
  for (let x = 0; x < data.length - 1; x += 2) {
    ...
    if (id.includes("sdMounted")) {
      ...
      if (platformPrefix == "Surveyor") {
        ...
        platformPrefix == "Facet L-Band" ||
        platformPrefix == "Facet L-Band Direct"
      ) {
        show("baseConfig");
        hide("sensorConfig");
        show("ppConfig");
    ...
    //Strings generated by RTK unit
      ...
      $("input[name=profileRadio][value=" + currentProfileNumber + "]").prop(
        "checked",
        true,
      );
      ...
      if (val > 0) ge("peerMACs").innerHTML = "";
      ...
      fileTableText +=
        ...
        lastFileName +
        "' name='fileID' class='form-check-input fileManagerCheck'></td>";
      fileTableText += "</tr>";
      ...
      if (messageData.length >= 3) {
        ...
        if (messageData.length == 4) {
          messageNameLabel =
            messageData[1] + "_" + messageData[2] + "_" + messageData[3]; //RTCM_4072_1
        }
      ...
      messageText +=
        "<label for='" +
        messageName +
        "' class='col-sm-4 col-6 col-form-label'>" +
        messageNameLabel +
        ":</label>";
      messageText +=
        "<div class='col-sm-4 col-4'><input type='number' class='form-control'";
      messageText += "id='" + messageName + "' value='" + messageRate + "'>";
      messageText +=
        "<p id='" + messageName + "Error' class='inlineError'></p>";
      messageText += "</div></div>";
  ...
  if (fullPageUpdate) {
    ...
    ge("enablePointPerfectCorrections").dispatchEvent(
...
function sendData() {
  ...
  clsElements = document.querySelectorAll(
    ".form-check-input:not(.fileManagerCheck), .form-radio",
  );
...
function validateFields() {
  ...
  checkElementString(
    ...
    1,
    49,
    "Must be 1 to 49 characters",
    "collapseProfileConfig",
  );
  ...
  checkElementValue(
    ...
    0.00012,
    10,
    "Must be between 0.00012 and 10Hz",
    "collapseGNSSConfig",
  );
  checkConstellations();
  ...
  if (ge("enableNtripClient").checked) {
    checkElementString(
      "ntripClient_CasterHost",
      1,
      45,
      "Must be 1 to 45 characters",
      "collapseGNSSConfig",
    );
    checkElementValue(
      "ntripClient_CasterPort",
      1,
      99999,
      "Must be 1 to 99999",
      "collapseGNSSConfig",
    ...
    checkElementCasterUser(
      "ntripClient_CasterHost",
      "ntripClient_CasterUser",
      "rtk2go.com",
      "@",
  ...
  if (platformPrefix != "Express Plus") {
    ...
    if (ge("enableNtripServer").checked == true) {
      ...
      checkElementString(
        "ntripServer_CasterUser_0",
        0,
        49,
        "Must be 0 to 49 characters",
        "collapseBaseConfig",
      );
      checkElementString(
        "ntripServer_CasterUserPW_0",
        0,
        49,
        "Must be 0 to 49 characters",
        "collapseBaseConfig",
  ...
  if (platformPrefix == "Express Plus") {
    if (ge("autoIMUmountAlignment").checked == false) {
      checkElementValue(
        ...
        0,
        360,
        "Must be between 0.0 to 360.0",
        "collapseSensorConfig",
      );
      checkElementValue(
        "imuPitch",
        -90,
        90,
        "Must be between -90.0 to 90.0",
        "collapseSensorConfig",
      );
      checkElementValue(
        "imuRoll",
        -180,
        180,
        "Must be between -180.0 to 180.0",
        "collapseSensorConfig",
      );
    }
  }
  ...
  checkCheckboxMutex(
    "enablePvtClient",
    "enablePvtServer",
    "TCP Client and Server can not be enabled at the same time",
    "collapseTCPUDPConfig",
  );
  ...
  if (ge("enableLogging").checked) {
    checkElementValue(
      ...
      "collapseSystemConfig",
    );
    checkElementValue(
      "maxLogLength_minutes",
      1,
      1051200,
      "Must be 1 to 1,051,200",
      "collapseSystemConfig",
  ...
  if (platformPrefix == "Reference Station") {
    if (ge("ethernetDHCP").checked == false) {
      checkElementIPAddress(
        "ethernetIP",
        "Must be nnn.nnn.nnn.nnn",
        "collapseEthernetConfig",
      );
      checkElementIPAddress(
        "ethernetDNS",
        "Must be nnn.nnn.nnn.nnn",
  ...
  if (platformPrefix != "Surveyor") {
    if (ge("enableExternalPulse").checked) {
      checkElementValue(
        ...
        1,
        60000000,
        "Must be 1 to 60,000,000",
        "collapsePortsConfig",
      );
      checkElementValue(
        "externalPulseLength_us",
        1,
        60000000,
        "Must be 1 to 60,000,000",
        "collapsePortsConfig",
...
function changeProfile() {
  ...
  if (errorCount == 1) {
    ...
    currentProfileNumber = document.querySelector(
      "input[name=profileRadio]:checked",
    ).value;
...
function checkConstellations() {
  if (
    ge("ubxConstellationsGPS").checked == false &&
    ...
    errorCount++;
  } else clearError("ubxConstellations");
}
...
function checkLatLong() {
  ...
  if (inputTypeLong != inputTypeLat) {
    ...
    ge(collapseID).classList.add("show");
    errorCount++;
    ge("detectedFormatText").innerHTML = printableInputType(
      CoordinateTypes.COORDINATE_INPUT_TYPE_INVALID_UNKNOWN,
    );
  } else ge("detectedFormatText").innerHTML = printableInputType(inputTypeLat);
}
...
function checkElementCasterUser(host, user, url, needs, errorText, collapseID) {
  if (ge(host).value.toLowerCase().includes(url)) {
    ...
    if (
      value.length < 1 ||
      value.length > 49 ||
      value.includes(needs) == false
    ) {
      ge(user + "Error").innerHTML = "Error: " + errorText;
      ge(collapseID).classList.add("show");
      errorCount++;
    } else clearError(user);
  } else clearError(user);
}
...
function resetToFactoryDefaults() {
  ge("factoryDefaultsMsg").innerHTML =
    "Defaults Applied. Please wait for device reset...";
  ...
  ge("dynamicModel").addEventListener("change", function () {
    if (ge("dynamicModel").value != 4 && ge("enableSensorFusion").checked) {
      ge("dynamicModelSensorFusionError").innerHTML =
        "<br>Warning: Dynamic Model not set to Automotive. Sensor Fusion is best used with the Automotive Dynamic Model.";
    } else {
      ge("dynamicModelSensorFusionError").innerHTML = "";
    }
  });
  ...
  if (errorCount == 0) {
    ...
    for (; index < recordsGeodetic.length; ++index) {
      ...
      if (ge("nicknameGeodetic").value == parts[0]) {
        recordsGeodetic[index] =
          nicknameGeodetic.value +
          " " +
          fixedLatText.value +
          " " +
          fixedLongText.value +
...
function adjustHAE() {
  ...
  if (haeMethod == 1) {
    ...
    hae =
      Number(ge("fixedHAE_APC").value) -
      (Number(ge("antennaHeight").value) / 1000 +
        Number(ge("antennaReferencePoint").value) / 1000);
    ge("fixedAltitude").value = hae.toFixed(3);
  } else {
    ge("fixedHAE_APC").disabled = true;
    ge("fixedAltitude").disabled = false;
    hae =
      Number(ge("fixedAltitude").value) +
      (Number(ge("antennaHeight").value) / 1000 +
        Number(ge("antennaReferencePoint").value) / 1000);
...
function getFileList() {
  if (showingFileList == false) {
    ...
    ge("fileManagerTable").innerHTML =
      "<table><tr align='left'><th>Name</th><th>Size</th><td><input type='checkbox' id='fileSelectAll' class='form-check-input fileManagerCheck' onClick='fileManagerToggle()'></td></tr></tr></table>";
    fileTableText = "";
...
function fileManagerDownload() {
  ...
  numberOfFilesSelected = document.querySelectorAll(
    "input[name=fileID]:checked",
  ).length;
  fileNumber = 0;
...
function sendFile() {
  ...
  var urltocall =
    "/file?name=" + selectedFiles[fileNumber].id + "&action=download";
...
function networkCount() {
  ...
  var wifiNetworks = document.querySelectorAll(
    "input[id^=wifiNetwork]" && "input[id$=SSID]",
  );
  for (let x = 0; x < wifiNetworks.length; x++) {
    if (wifiNetworks[x].value.length > 0) count++;
  }
...
function checkNewFirmware() {
  ...
  else settingCSV += "enableRCFirmware,false,";
...
function identifyInputType(userEntry) {
  ...
  if (decimalCount > 1)
    return CoordinateTypes.COORDINATE_INPUT_TYPE_INVALID_UNKNOWN; //Just no. 40.09033470 is valid.
  if (spaceCount > 2)
    return CoordinateTypes.COORDINATE_INPUT_TYPE_INVALID_UNKNOWN; //Only 0, 1, or 2 allowed. 40 05 25.2049 is valid.
  if (dashCount > 3)
    return CoordinateTypes.COORDINATE_INPUT_TYPE_INVALID_UNKNOWN; //Only 0, 1, 2, or 3 allowed. -105-11-05.1629 is valid.
  if (lengthOfLeadingNumber > 7)
    return CoordinateTypes.COORDINATE_INPUT_TYPE_INVALID_UNKNOWN; //Only 7 or fewer. -1051105.188992 (DDDMMSS or DDMMSS) is valid
  ...
  if (
    spaceCount == 0 &&
    dashCount == 0 &&
    (lengthOfLeadingNumber == 7 || lengthOfLeadingNumber == 6)
  ) //DDMMSS.ssssss
  {
    ...
    if (userEntry.indexOf(".") == -1)
      coordinateInputType =
        CoordinateTypes.COORDINATE_INPUT_TYPE_DDMMSS_NO_DECIMAL;
    ...
    spaceCount == 0 &&
    dashCount == 0 &&
    (lengthOfLeadingNumber == 5 || lengthOfLeadingNumber == 4)
  ) //DDMM.mmmmmmm
  {
  ...
  } else if (dashCount == 2) //DD-MM-SS.ssss
  {
    ...
    if (userEntry.indexOf(".") == -1)
      coordinateInputType =
        CoordinateTypes.COORDINATE_INPUT_TYPE_DD_MM_SS_DASH_NO_DECIMAL;
  ...
  } else if (spaceCount == 2) //DD MM SS.ssssss
  {
    ...
    if (userEntry.indexOf(".") == -1)
      coordinateInputType =
        CoordinateTypes.COORDINATE_INPUT_TYPE_DD_MM_SS_NO_DECIMAL;
...
function convertInput(coordinate, coordinateInputType) {
  ...
  if (coordinateInputType == CoordinateTypes.COORDINATE_INPUT_TYPE_DD) {
    ...
    if (coordinateInputType == CoordinateTypes.COORDINATE_INPUT_TYPE_DDMM)
      ...
      coordinateString = longitudeDegrees + "°" + coordinate + "'";
    else coordinateString = longitudeDegrees + " " + coordinate;
  } else if (
    coordinateInputType == CoordinateTypes.COORDINATE_INPUT_TYPE_DD_MM_SS ||
    coordinateInputType == CoordinateTypes.COORDINATE_INPUT_TYPE_DDMMSS ||
    coordinateInputType ==
    ...
    if (coordinateInputType == CoordinateTypes.COORDINATE_INPUT_TYPE_DDMMSS)
      coordinateString =
        longitudeDegrees + "" + longitudeMinutes + "" + coordinate;
    else if (
      coordinateInputType == CoordinateTypes.COORDINATE_INPUT_TYPE_DD_MM_SS_DASH
    )
      coordinateString =
      ...
      coordinateString =
        longitudeDegrees + "" + longitudeMinutes + "" + Math.round(coordinate);
    else if (
      coordinateInputType ==
      CoordinateTypes.COORDINATE_INPUT_TYPE_DD_MM_SS_NO_DECIMAL
    )
